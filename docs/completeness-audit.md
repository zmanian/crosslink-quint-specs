...
| Weighted voting power | `VotingPowerOf`, `QuorumVotingPower`; `test:weighted`; `CrosslinkFatPointerFormat.qnt` | covered | Production fat-pointer signer-vector, exact wire-envelope, and producer-round-data derivation shapes are modeled; broader production integration remains open. |
...
| Accountability witnesses | `ConflictingCommitsAccountable`, nil/value equivocation and invalid-unlock witnesses; `CrosslinkFatPointerFormat.qnt` duplicate-pubkey, removed-validator, wrong-payload, cross-height replay, low-power, trailing-byte, truncated-wire, and producer-round-data derivation witnesses; `docs/accountability.md` | covered | Production slashing evidence formats remain open. |
...
stream, BFT-block-shape, fat-pointer-format, and evidence-composition models.
