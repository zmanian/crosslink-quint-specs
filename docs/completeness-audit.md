...
Tenderlink vote packet, consensus packet, nonce/ack transport, and status
packet formats.
...
packets, Tenderlink nonce/ack transport, Tenderlink status packets, and
heighted authenticated gossip transport models.
...
- link message-authentication and evidence-gossip models to concrete production
  ...
  gating and the status packet format are modeled, but Malachite protobuf
  proposal vectors and full production gossip integration remain open;
