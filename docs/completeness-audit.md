...
| Tendermint lock and valid-value rules | `lockedValue`, `lockedRound`, `validValue`, `validRound`, `ProposalUnlocksCurrentLock`; `ProposalFor` reuses only real `validValue`/`validRound` state; per-height lock/valid state and height-scoped valid-round unlock in `CrosslinkHeightedRound.qnt`; `CrosslinkTenderlinkProposalPolEvidence.qnt` bridges non-nil `validRound` proposal chunks to canonical prevote packet evidence | partial | Needs broader production finality/auth/evidence integration and full validator-set-size production threshold fixtures. |
| Valid-round/POL evidence | `LocalValidRoundJustified`, `CorrectProposalValidRoundSound`; `CrosslinkTenderlinkProposalPolEvidence.qnt`; `test:valid-round`; `test:tenderlink-proposal-pol-evidence`; `unjustifiedHeightedValidRoundProposalPrevotesNilTest`; `justifiedHeightedValidRoundUnlocksOlderLockTest`; `verify:tenderlink-proposal-pol-evidence-safety` | covered | Full production gossip integration and larger/weighted validator-set packet fixtures remain open. |
...
Tenderlink vote packet, proposal/POL evidence, consensus packet, nonce/ack transport, status
...
packets, Tenderlink nonce/ack transport, Tenderlink status packets,
...
- link message-authentication and evidence-gossip models to concrete production
  ...
  compact consensus packet layouts are now pinned. Known-peer nonce/ack replay
  gating and the status packet format are modeled. Initial Malachite proposal,
  liveness, and sync protobuf vectors are pinned, first proposal/liveness/sync
  gossip-envelope bridges exist, and a shared Malachite gossip router plus
  verifier-friendly router safety slice compose those channel namespaces;
  broader production gossip integration remains open;
