...
| Shared protocol core with explicit variants | Shared `CrosslinkResampling` core with baseline/resampling modules, plus standalone `CrosslinkDynamicSigmaModel`, `CrosslinkDynamicSigmaHeadSamplingModel`, and `CrosslinkDynamicSigmaHeightedRoundModel` for the third dynamic-sigma variant | partial | Dynamic sigma is first-class in the spec surface and now bridged through head-sigma sampling into the heighted round machine, but not yet composed with auth/evidence/finality or production consensus-parameter encoding. |
...
| Dynamic-sigma Crosslink variant | `CrosslinkDynamicSigma.qnt`; `CrosslinkDynamicSigmaHeadSampling.qnt`; `CrosslinkDynamicSigmaHeightedRound.qnt`; `CrosslinkDynamicSigmaModel`; `CrosslinkDynamicSigmaHeadSamplingModel`; `CrosslinkDynamicSigmaHeightedRoundModel`; `test:dynamic-sigma`; `test:dynamic-sigma-head-sampling`; `test:dynamic-sigma-heighted-round`; `verify:dynamic-sigma-safety`; `verify:dynamic-sigma-head-sampling-safety`; `verify:dynamic-sigma-heighted-round-safety`; `verify:extended:dynamic-sigma`; `verify:extended:dynamic-sigma-head-sampling`; `verify:extended:dynamic-sigma-heighted-round` | partial | First controller model treats dynamic sigma as a third Crosslink variant, keeps sigma fixed for the active BFT height, updates only at height boundaries from committed telemetry, validator/network coverage evidence, and Crosslink-participating PoW hash-power percentage, refuses to raise sigma for same-branch block-arrival failures, and applies raise/decrease hysteresis. Low participating hash power contributes to long-reorg/ambiguous-failure pressure and prevents sigma relaxation. The head-sampling and heighted-round bridges now check `head - sigma(h)` semantics across future-height sigma changes, including proposals, value precommits, nil-round resampling, and a height transition. It is not yet composed with auth/evidence/finality or production consensus-parameter encoding. |
...
PoW-reorg stress, dynamic-sigma controller, dynamic-sigma head-sampling,
dynamic-sigma heighted-round, head-sigma stream, BFT-block-shape, BFT-block
