...
| `CrosslinkPowStochasticAssumptions.qnt` / `UsesZebraPostBlossomSpacing` | `zebra-chain/src/parameters/network_upgrade.rs::POST_BLOSSOM_POW_TARGET_SPACING`; Crosslink sigma/GST/reorg-tail parameter analysis | This executable assumption profile pins the normal PoW-arrival denominator to Zebra's 75-second post-Blossom target spacing, models the prevote-to-precommit exposure window as a validator-set-size/GST numerator, and keeps the long-reorg-tail numerator explicit by sigma. The numeric tail and propagation-risk inputs are still assumptions to replace with measured or analysis-backed distributions. |
...
- Replace the current PoW stochastic-assumption numerators with measured or
  analysis-backed distributions for normal block arrivals, propagation races,
  GST scaling, and long-tail reorg depth.
