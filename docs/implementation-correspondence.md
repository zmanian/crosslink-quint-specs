...
| `CrosslinkFatPointerProductionVectors.qnt` / `SerializerCountSliceIsBytes44To46` / `StreamingDeserializerMatchesSerializer` / `PrototypeTryFromBytesGapRecorded` | `zebra-chain/src/block/header.rs::FatPointerToBftBlock::{to_bytes, zcash_deserialize, try_from_bytes}`; `zebra-crosslink/src/{lib.rs,malctx.rs}::FatPointerToBftBlock2::try_from_bytes`; `forks/tenderlink/src/lib.rs::FatPointerToBftBlock3::try_from_bytes`; `crosslink-test-data/test_pos_block_*.bin`; `fixtures/production-bft-block-vectors.json` | Pins exact implementation-linked vectors for 0-4 signatures: 46, 142, 238, 334, and 430 bytes. The streaming serializer/deserializer agree on count bytes 44..46. The checked-in BFT fixture manifest anchors a previous fat pointer at byte 8 and a trailing fat pointer at byte 4619, each with one signer, count bytes at offsets 52 and 4663, signer pubkeys at 54 and 4665, and vote signatures at 86 and 4697. The current prototype helper `try_from_bytes` uses a reversed count slice (`bytes[44..2]`) in multiple copies; the spec records that as an implementation gap rather than treating it as the protocol layout. |
