...
- `CrosslinkValidatorScaleFinalityProgressContract.qnt` composes that scale
  ...
  of the stable candidate while rejecting the competing fork candidate.
...
- `CrosslinkDynamicSigmaProposalEvidenceFormat.qnt` pins the concrete
  proposal-carried dynamic-sigma evidence payload implemented in
  `zebra-crosslink/src/dynamic_sigma.rs`: raw telemetry counters, conservative
  margins, and selected sigma serialized as a 123-byte little-endian payload.
  It uses split u128 high/low words to keep Rust-backed Quint tests finite
...
- `CrosslinkDynamicSigmaPrototypeDecodeGate.qnt` pins the adjacent
  implementation feature gate in `decode_tenderlink_payload`: tagged
  dynamic-sigma bytes are rejected while `dynamic_sigma_prototype` is disabled,
  never fall back to fixed-sigma decoding, and report the evidence-selected
  confirmation depth only after the prototype gate is enabled.
...
- `CrosslinkBftBlockValidationGap.qnt` makes the current implementation
  ...
  inputs. It also records the deserialization bypass and repaired deserializer
  target: counted envelopes may be read up to the 2048-header cap, but accepted
  BFT blocks must still satisfy the constructor rule.
...
- `CrosslinkFixtureAuthenticatedEvidence.qnt` bridges generated checked-in
  ...
  with `pubkey || payload` shape and real Ed25519 verification pinned by the
...
- `CrosslinkValidatorScaleFinalityProgressContractModel` extends that finite
  graph through Crosslink finality. It checks that the same validator-scale
  resampling path can advance a linear finalized prefix to the stable candidate
  under weak fairness while keeping the competing fork candidate unfinalized.
...
- `CrosslinkDynamicSigmaProposalEvidenceFormatModel` checks the concrete
  Zebra proposal-evidence byte boundary: 123-byte payload length, u128 field
...
- `CrosslinkDynamicSigmaPrototypeDecodeGateModel` checks Zebra's prototype
  decode gate for that payload boundary: default config rejects magic-prefixed
...
- The remaining multi-height work is to connect the heighted auth, evidence,
  ...
  checked-in fixture transport, proposal-transport, and finality boundary,
  ...
  proposal payload and its prototype decode gate, quorum-signed production-byte
  gossip, node-config application, proposals, precommits, fat-pointer evidence, and finality over
